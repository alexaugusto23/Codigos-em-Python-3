...
class TestPartOne(unittest.TestCase):
    ...
    def test_05_email(self):
        self.assertEqual(email(agenda1, 'lucas'), 'lucas.goncalves@faculdadeimpacta.com.br')
        self.assertEqual(email(agenda1, 'maria'), 'maria.aparecida@exemplo.com')
        self.assertEqual(email(agenda1, 'fulano'), 'fulano@exemplo.com')
        self.assertEqual(email(agenda1, 'fulano'), 'fulano@exemplo.com')

    def test_06_telefone_principal(self):
        self.assertEqual(telefone_principal(agenda1, 'lucas (professor)'), 11999888999)
        self.assertEqual(telefone_principal(agenda1, 'maria'), 84999777444)
        self.assertEqual(telefone_principal(agenda1, 'jadir'), 1177788899)
        self.assertEqual(telefone_principal(agenda1, 'a'), 1122233344)
        self.assertEqual(telefone_principal(agenda1, 'a'), 11321321321)
        self.assertEqual(telefone_principal(agenda1, 'ana'), 1123232323)
        self.assertEqual(telefone_principal(agenda1, 'fulano'), 11777888999)
